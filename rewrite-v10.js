...
const code = `(function() {
  ...
  var winW, winH;
  var planeWidth, planeHeight, verticalTravel, fixedX;

  var SCENE_CONFIG = {
    mobileSmall: { maxW: 480, dpr: 1.25, zoomStart: 1.05, zoomEnd: 1.0, margin: 1.08, smoothing: 0.14 },
    mobileLarge: { maxW: 767, dpr: 1.25, zoomStart: 1.05, zoomEnd: 1.0, margin: 1.08, smoothing: 0.14 },
    tablet: { maxW: 1024, dpr: 1.5, zoomStart: 1.06, zoomEnd: 1.0, margin: 1.10, smoothing: 0.12 },
    desktop: { maxW: Infinity, dpr: 2.0, zoomStart: 1.08, zoomEnd: 1.0, margin: 1.15, smoothing: 0.10 }
  };
  ...
  function init() {
    // Safety check 1: THREE exists
    if (typeof THREE === 'undefined') {
      fallback();
      return;
    }
    // Safety check 2: Reduced Motion
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      fallback();
      return;
    }

    // Pre-populate reel if not already
  ...
  function calculateGeometry() {
    ...
    var imgAspect = 16 / 9;
    if (imgTexture && imgTexture.image) {
      imgAspect = imgTexture.image.width / imgTexture.image.height;
    }
    
    var viewportAspect = winW / winH;
    
    // We want the plane to cover the screen AND be slightly taller for vertical travel.
    var isMobile = config.maxW <= 767;
    
    // Step 1: Ensure it covers width and height.
    planeHeight = winH * config.margin;
    planeWidth = planeHeight * imgAspect;
    
    // If it doesn't cover width, scale it up so width fits.
    if (planeWidth < winW) {
      planeWidth = winW;
      planeHeight = planeWidth / imgAspect;
    }
    
    verticalTravel = Math.max(0, (planeHeight - winH) / 2);
    
    // Fixed X to keep elevator visible (right side of image usually has it).
    // The image has the elevator in the center-right.
    if (isMobile) {
      // If plane is much wider than screen, shift camera left so right side of image is visible
      fixedX = (planeWidth - winW) / 2 * 0.45; // Pos X looks at left side, we want center-right, so X should be positive? 
      // Actually camera at +X looks at right side. Wait, if camera is at +X, we see the right side.
      // Let's use 0.4 for mobile to keep the elevator visible.
    } else {
  ...
  function buildScene() {
    ...
    var frag = "uniform sampler2D tDiffuse; uniform float uProgress; uniform vec2 uResolution; varying vec2 vUv; " +
               ...
               "texColor.rgb += hash(gl_FragCoord.xy) * 0.025; " + // Fixed static screen-space grain
  ...
  function frame() {
    ...
    // TRUE VERTICAL SCROLL MOVEMENT
    var verticalProgress = smoothstep(0.06, 0.96, p);
    
    // Start at bottom of the image (since image moves down, camera Y starts low)
    var startY = -verticalTravel;
    var endY = verticalTravel;
    var targetY = startY + (endY - startY) * verticalProgress;
    
    // Restrained zoom
    // Scene 01: Slightly closer (config.zoomStart)
    // Scene 03/04: Gradual pullback to 1.0 (zoomEnd)
    var zoomProgress = smoothstep(0.18, 0.62, p);
    camera.zoom = config.zoomStart - (config.zoomStart - config.zoomEnd) * zoomProgress;
    camera.updateProjectionMatrix();

    // NO HORIZONTAL MOVEMENT
    camera.position.x = fixedX;
    camera.position.y = targetY;
    camera.lookAt(fixedX, targetY, 0);
    ...
    if (reel && ldDir) {
      ...
      // Arrow direction logic (reversed on upward scroll)
      if (deltaP > 0.001) {
